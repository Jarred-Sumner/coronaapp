...
class TrackableVideoSource : TrackableMediaSource {
  ...
  func start(player: AVQueuePlayer, autoPlay: Bool = false) {
    ...
    mediaSource.loadAsset { [weak self] asset in
      ...
      self?.periodicObserver = player.addPeriodicTimeObserver(forInterval: CMTimeMakeWithSeconds( TrackableMediaSource.periodicInterval, preferredTimescale: 60), queue: .main) {  [weak self] time in
